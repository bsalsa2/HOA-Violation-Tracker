...
export default function ResidentsTab({ residents, onAdd, onImport, onDelete, onRestore, onViewViolations, onSeedDemo, seeding }) {
  const [query, setQuery] = useState('')
  const [showArchived, setShowArchived] = useState(false)

  const archivedCount = useMemo(() => residents.filter((r) => r.archived_at).length, [residents])

  const filtered = useMemo(() => {
    let list = showArchived ? residents : residents.filter((r) => !r.archived_at)
        ...
        <div className="flex gap-2 shrink-0 items-center">
          {archivedCount > 0 && (
            <button
              ...
              className={`px-3 py-2 text-xs rounded-lg border transition-colors ${showArchived ? 'bg-white/[0.06] text-slate-300 border-white/20' : 'text-slate-500 border-white/10 hover:border-white/20'}`}
              title="Archived residents keep their violation history"
            >
      ...
      <div className="divide-y divide-white/[0.05] max-h-[calc(100vh-20rem)] overflow-y-auto">
        {filtered.length === 0 ? (
          ...
          filtered.map((r) => {
            ...
            return (
              <div key={r.id} className={`flex items-center justify-between px-5 py-3 hover:bg-white/[0.03] group transition-colors ${isArchived ? 'opacity-60' : ''}`}>
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {isArchived && (
                      <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-slate-500/10 text-slate-400 border border-slate-500/25 font-medium shrink-0">
                        Archived
                      </span>
                    )}
                    {!isArchived && repeat && (
                ...
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  {isArchived ? (
                    <button
                      onClick={() => onRestore(r)}
                      className="px-2.5 py-1 text-xs text-emerald-400 border border-emerald-500/25 hover:bg-emerald-500/10 rounded-lg transition-colors"
                    ...
                    <button
                      onClick={() => onDelete(r.id, r.name, r.violation_count || 0)}
                      className="opacity-0 group-hover:opacity-100 focus:opacity-100 p-1.5 text-slate-500 hover:text-red-400 rounded-lg hover:bg-red-400/10 transition-all"
                      title={(r.violation_count || 0) > 0 ? 'Archive resident (history preserved)' : 'Delete resident'}
                      aria-label={`${(r.violation_count || 0) > 0 ? 'Archive' : 'Delete'} ${r.name}`}
                    >
