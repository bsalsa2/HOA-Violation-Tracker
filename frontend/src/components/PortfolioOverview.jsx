...
import { InviteModal } from './AccountMenu'
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        >
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#60a5fa] to-[#2563eb] ring-1 ring-white/15 flex items-center justify-center text-sm font-bold text-slate-100 shrink-0" style={{ boxShadow: 'inset 0 1px 0 0 rgba(255,255,255,0.25)' }}>
            {(hoa.name || '?').replace(/[^a-zA-Z ]/g, '').split(' ').map((p) => p[0]).slice(0, 2).join('').toUpperCase()}
          </div>
          <div className="min-w-0">
            <h3 className="text-slate-100 font-semibold truncate group-hover:text-slate-100">{hoa.name}</h3>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          {!isConfigured && (
            <span className="text-[10px] px-2 py-0.5 rounded-full bg-[#3b82f6]/12 text-[#60a5fa] border border-[#3b82f6]/25 font-medium">
              Setup needed
            </span>
          )}
          <button
      ...
      <div className="grid grid-cols-3 gap-2 mt-4">
        <div className="bg-white/[0.03] ring-1 ring-white/[0.05] rounded-xl p-2.5">
          <p className="text-lg font-bold text-slate-100 leading-none"><CountUp value={hoa.total_residents} /></p>
          <p className="text-[10px] text-slate-500 mt-1">Residents</p>
        </div>
        <div className="bg-white/[0.03] ring-1 ring-white/[0.05] rounded-xl p-2.5">
          <p className="text-lg font-bold text-[#60a5fa] leading-none"><CountUp value={hoa.open_violations} /></p>
      ...
      <button onClick={() => onOpen(hoa)} className="flex items-center gap-1 mt-4 text-xs text-[#60a5fa] hover:text-[#60a5fa] transition-colors">
        Open dashboard
    ...
    <div className="min-h-screen bg-transparent text-slate-100">
      <header className="relative bg-[#0b0e14]/85 backdrop-blur-xl border-b border-white/[0.06] px-6 py-3 sticky top-0 z-20 shadow-[0_10px_36px_-18px_rgba(0,0,0,0.7)]">
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 bg-gradient-to-br from-[#60a5fa] to-[#2563eb] rounded-xl flex items-center justify-center ring-1 ring-white/15" style={{ boxShadow: '0 2px 8px rgba(0,0,0,0.08), inset 0 1px 0 0 rgba(255,255,255,0.3)' }}>
              <svg className="w-4 h-4 text-slate-100" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>
            </div>
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setModal('invite')} className="px-3 py-1.5 text-xs text-slate-400 hover:text-slate-100 border border-white/10 hover:border-white/20 hover:bg-white/[0.06] rounded-lg transition-colors">Invite links</button>
      ...
      <main className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {hoas.map((h) => (<ClientCard key={h.id} hoa={h} onOpen={onOpen} onSettings={onEditClient || onOpen} />))}
          <button
            onClick={onAddClient}
            className="border-2 border-dashed border-white/10 hover:border-[#3b82f6]/40 hover:bg-[#3b82f6]/[0.04] rounded-2xl p-5 flex flex-col items-center justify-center gap-2 text-slate-500 hover:text-[#60a5fa] transition-all min-h-[180px] group"
          >
            <div className="w-11 h-11 rounded-xl bg-white/[0.03] ring-1 ring-white/[0.06] flex items-center justify-center group-hover:ring-[#3b82f6]/40 transition-all">
