...
import { residentAPI, violationAPI, hoaAPI, adminAPI } from '../api'
    ...
    <Modal title="Add Client (HOA)" subtitle="Add another community to your portfolio" onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex gap-3 pt-2">
          <button type="submit" disabled={loading} className="flex-1 py-2.5 bg-gradient-to-b from-[#3b82f6] to-[#2563eb] hover:from-[#60a5fa] hover:to-[#3b82f6] shadow-lg shadow-[#2563eb]/40 active:scale-[.98] disabled:opacity-60 text-white font-semibold rounded-lg transition-colors">
            {loading ? 'Adding…' : 'Add Client'}
          </button>
          <button type="button" onClick={onClose} className="px-5 py-2.5 border border-slate-700 text-slate-400 hover:bg-slate-800 rounded-lg transition-colors">Cancel</button>
        </div>
      </form>
    </Modal>
  )
    ...
    return (
      <Modal title="Import Results" onClose={() => onDone(result.added, result.errors || [])}>
        <div className="space-y-4">
          ...
          <button onClick={() => onDone(result.added, result.errors || [])} className="w-full py-2.5 bg-gradient-to-b from-[#3b82f6] to-[#2563eb] hover:from-[#60a5fa] hover:to-[#3b82f6] shadow-lg shadow-[#2563eb]/40 active:scale-[.98] text-white font-semibold rounded-lg transition-colors">Done</button>
        </div>
      </Modal>
    )
  }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input type="file" accept=".csv" onChange={(e) => setFile(e.target.files[0])}
              className="w-full text-sm text-slate-400 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-[#3b82f6]/15 file:text-[#60a5fa] hover:file:bg-[#3b82f6]/25 file:cursor-pointer file:transition-colors" required />
          </div>
          <div className="flex gap-3">
            <button type="submit" disabled={loading || !file} className="flex-1 py-2.5 bg-gradient-to-b from-[#3b82f6] to-[#2563eb] hover:from-[#60a5fa] hover:to-[#3b82f6] shadow-lg shadow-[#2563eb]/40 active:scale-[.98] disabled:opacity-60 text-white font-semibold rounded-lg transition-colors">
  ...
  return (
    <Modal title="Edit HOA Settings" onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-4 max-h-96 overflow-y-auto">
        <div className="bg-[#3b82f6]/[0.08] border border-[#3b82f6]/20 rounded-lg p-3 text-xs text-[#60a5fa]">
